...
namespace Rowlan.Fullscreen
{
    ...
    public static class FullscreenStateGuard
    {
            ...
            Action<int> setGameViewTargetDisplay)
        {
            ...
            var persistedId = SessionState.GetInt(KeyInstanceId, 0);
            ...
            if (GameViewType != null && ShowToolbarProperty != null)
                foreach (var obj in Resources.FindObjectsOfTypeAll(GameViewType))
                {
                    ...
                    try
                    {
                        var toolbar = (bool)ShowToolbarProperty.GetValue(win);
                        if (!toolbar)
                        {
                            if (FullscreenSettings.DebugLogging)
                                Debug.Log(
                                    $"[Fullscreen] Closing orphaned toolbar-hidden GameView (id={win.GetInstanceID()}).");
                            try
                            {
                                win.Close();
                            }
            ...
            var displayToRestore = SessionState.GetInt(KeyOriginalDisplay, 0);
            EditorApplication.delayCall += () =>
            {
                try
                {
                    setGameViewTargetDisplay?.Invoke(displayToRestore);
                }
                catch
                {
                }
            ...
            var savedVSync = SessionState.GetInt(KeyOriginalVSync, -1);
            if (savedVSync >= 0)
