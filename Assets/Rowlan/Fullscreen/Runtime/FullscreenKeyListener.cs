...
namespace Rowlan.Fullscreen
{
    ...
    public class FullscreenKeyListener : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (!ResolveAndValidateKeybinds()) enabled = false;
        }
        ...
        {
            var toggleKey = FullscreenKeybinds.ToggleKey;
            var exitKey = FullscreenKeybinds.ExitKey;
            var resetKey = FullscreenKeybinds.ResetKey;
            ...
            {
                Debug.LogWarning(
                    $"[Fullscreen] Invalid keybinds (toggle='{toggleKey}', exit='{exitKey}', reset='{resetKey}'). Key listener disabled.");
                return false;
            }
