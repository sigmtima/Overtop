...
namespace NavMeshPlus.Extensions
{
    ...
    internal class NavMeshBuilder2d
    {
        ...
        {
            foreach (var it in builder.Root) CollectSources(it, sources, builder);
            ...
            NavMeshBuilder2dState builder)
        {
            ...
            if (modifierTilemap && !modifierTilemap.AffectsAgentType(builder.agentID)) return;
            ...
            for (var i = bound.xMin; i < bound.xMax; i++)
            for (var j = bound.yMin; j < bound.yMax; j++)
            {
                var src = new NavMeshBuildSource();
                src.area = area;
                ...
                if (modifierTilemap && modifierTilemap.TryGetTileModifier(vec3int, tilemap, out var tileModifier))
            ...
            Vector3 size, Mesh sharedMesh, Quaternion rot, ref NavMeshBuildSource src)
        {
            if (!builder.overrideByGrid && tilemap.GetColliderType(vec3int) == Tile.ColliderType.Sprite)
            {
                ...
                if (sprite != null)
                {
                    var mesh = builder.GetMesh(sprite);
        ...
        internal static void sprite2mesh(Sprite sprite, Mesh mesh)
        {
            ...
            for (var i = 0; i < sprite.triangles.Length; i++) tri[i] = sprite.triangles[i];
            mesh.triangles = tri;
        }
